...
try:
    with open(config_path, "r") as f:
        # yaml.safe_load converts YAML lists to Python lists and YAML maps to Python dicts
        data = yaml.safe_load(f)

        # Case 1: Dict with a "commands" key containing a list (Matches your commands.yaml)
        if (
...
def __getattr__(name: str) -> Type[HoneyPotCommand]:
    ...
    if name.startswith("Command_"):
        ...
        mock_output = COMMAND_MAP.get(cmd_name, f"{cmd_name}: execution successful.\n")
